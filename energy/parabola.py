...
def suppose_parabola(parabola_arg, candidates, logger):
    if len(parabola_arg) != 3:
        raise ValueError("Input error: ", parabola_arg)
    logger.info("Initial arguments: x_1 = %f, x_2 = %f, x_3 = %f" % (parabola_arg[0], parabola_arg[1], parabola_arg[2]))
    logger.info("Initial energies: e_1 = %f, e_2 = %f, e_3 = %f" % (candidates[0], candidates[1], candidates[2]))
    ...
    logger.info("Minimum of parabola: %f Energy: %f"% (parabola_min_arg, E_new))
    parab_minimum = namedtuple("parab_minimum", ["E", "arg", "grad_0", "grad_1", "grad_2"])

    logger.info("Calculated coefs: \n\ta = {0} \n\tb = {1} \n\tc = {2}"
                                .format(
                                    str(parabola_coefficients['a']),
                                    str(parabola_coefficients['b']),
                                    str(parabola_coefficients['c'])))
